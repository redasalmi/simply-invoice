...
import { check, customType, index, real, sqliteTable, text } from 'drizzle-orm/sqlite-core'
...
export const companiesTable = sqliteTable(
  'companies_table',
  {
    ...
    status: text('status', { enum: ['active', 'inactive'] })
      .notNull()
      .default('active'),
    additionalInformation: customJson<Array<PortableTextBlock>>('additional_information'),
    addressId: text('address_id', { length: 36 })
      .notNull()
...
export const invoicesTable = sqliteTable(
  ...
  (table) => [
    check('identifier_type_check', sql`identifier_type IN ('incremental', 'random', 'manual')`),
    check('invoice_status_check', sql`status IN ('draft', 'sent', 'paid', 'overdue', 'cancelled')`),
...
export const userSettingsTable = sqliteTable(
  ...
  (table) => [
    ...
    check('setting_type_check', sql`setting_type IN ('number', 'string', 'boolean', 'json')`),
