...
export const companyCreateWithAddressSchema = v.omit(
  createInsertSchema(companiesTable, {
    ...
    email: (schema) =>
      v.pipe(schema, v.nonEmpty('Email is required'), v.email('Invalid email address')),
  }),
  ['addressId']
)
...
export const customerUpdateWithAddressSchema = v.object({
  ...
  customerId: v.pipe(v.string(), v.nonEmpty('Customer ID is required'), v.uuid()),
})
...
export const taxDeleteSchema = v.pipe(v.string(), v.nonEmpty('Tax ID is required'), v.uuid())
...
export const userSettingUpdateSchema = createInsertSchema(userSettingsTable, {
  ...
  settingValue: (schema) => v.pipe(schema, v.nonEmpty('Setting value is required')),
  settingType: (schema) => v.pipe(schema, v.nonEmpty('Setting type is required')),
})
