...
import { type InputProps, Input as UIInput } from '~/renderer/components/ui/input'
import { type LabelProps, Label as UILabel } from '~/renderer/components/ui/label'
import {
...
export function Input(props: Omit<InputProps, 'id' | 'aria-invalid' | 'aria-describedby'>) {
...
export function Root({ className, children, errors, ...props }: RootProps) {
  ...
  return (
    <div className={cn('flex flex-col gap-2', className)} {...props}>
      <FormFieldContext.Provider value={{ inputId, errorId, errors, hasErrors }}>
        {children}
