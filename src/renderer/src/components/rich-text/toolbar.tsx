...
import { type SchemaDefinition, schemaDefinition } from '~/renderer/components/rich-text/schema'
...
function ToolbarButton({ name, icon, active, onClick }: ToolbarButtonProps) {
  ...
  return (
    <Button
      aria-label={name}
      className={cn('flex size-9 items-center justify-center p-0', active && 'bg-gray-500')}
      onClick={() => {
        onClick()
...
function StyleButtons() {
  return schemaDefinition.styles.map((style) => <StyleButton key={style.name} {...style} />)
}

function ListButtons() {
  return schemaDefinition.lists.map((list) => <ListButton key={list.name} {...list} />)
}
