...
import type { Address, AddressCreateFlatErrors, AddressUpdateFlatErrors } from '~/types'

interface AddressFormFieldsProps extends Omit<React.ComponentPropsWithRef<'div'>, 'children'> {
  address?: Address
  errors?: AddressUpdateFlatErrors | AddressCreateFlatErrors
}

export function AddressFormFields({ address, errors, ...props }: AddressFormFieldsProps) {
  return (
    <div {...props}>
      {address?.addressId ? (
        <input name="address.addressId" type="hidden" value={address.addressId} />
      ) : null}

      <FormField.Root errors={errors?.nested?.address1}>
        <FormField.Label>Address 1</FormField.Label>
        <FormField.Input defaultValue={address?.address1} name="address.address1" type="text" />
      ...
      <FormField.Root errors={errors?.nested?.city}>
        ...
        <FormField.Input defaultValue={address?.city} name="address.city" type="text" />
        <FormField.ErrorMessage />
      </FormField.Root>

      <FormField.Root errors={errors?.nested?.country}>
        <FormField.Label>Country</FormField.Label>
        <FormField.Input defaultValue={address?.country} name="address.country" type="text" />
        <FormField.ErrorMessage />
      </FormField.Root>
      ...
      <FormField.Root errors={errors?.nested?.zip}>
        ...
        <FormField.Input defaultValue={address?.zip} name="address.zip" type="text" />
        <FormField.ErrorMessage />
      </FormField.Root>
    </div>
  )
