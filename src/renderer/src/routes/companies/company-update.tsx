...
import type { UpdateAddressInput, UpdateCompanyWithAddressInput } from '~/types'
...
export async function companyUpdateAction({ request }: ActionFunctionArgs) {
  ...
  const result = await window.api.db.companies.updateWithAddress(company, address)
      ...
      <section>
        <div>
          <p className="m-12">
            ...
            <Link to="/companies">here</Link> to navigate back to your companies list.
          </p>
        </div>
      </section>
    )
  ...
  return (
    <section>
      <Form className="flex flex-col gap-4" method="post" ref={formRef}>
        <input name="company.companyId" type="hidden" value={company.companyId} />

        <FormField.Root errors={actionData?.errors?.company?.nested?.name}>
          <FormField.Label>Name</FormField.Label>
          <FormField.Input defaultValue={company.name} name="company.name" type="text" />
          <FormField.ErrorMessage />
        </FormField.Root>

        <FormField.Root errors={actionData?.errors?.company?.nested?.email}>
          <FormField.Label>Email</FormField.Label>
          <FormField.Input defaultValue={company.email} name="company.email" type="text" />
          <FormField.ErrorMessage />
        </FormField.Root>

        <FormField.Root errors={actionData?.errors?.company?.nested?.phone}>
          <FormField.Label>Phone</FormField.Label>
          <FormField.Input defaultValue={company.phone ?? ''} name="company.phone" type="text" />
          <FormField.ErrorMessage />
        </FormField.Root>

        <FormField.Root errors={actionData?.errors?.company?.nested?.taxId}>
          <FormField.Label>Tax ID</FormField.Label>
          <FormField.Input defaultValue={company.taxId ?? ''} name="company.taxId" type="text" />
          <FormField.ErrorMessage />
        </FormField.Root>

        <Select
          defaultSelectedItem={statusOptions.find((option) => option.value === company.status)}
        ...
        <div>
          <div>
            ...
            <p className="mb-2 block text-sm">Add additional information about the company</p>
          </div>
