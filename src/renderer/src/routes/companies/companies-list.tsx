...
export async function companiesListLoader({ request, context }: LoaderFunctionArgs) {
  ...
  return {
    companies: await window.api.db.companies.get(cursor, paginationType, itemsPerPage),
  }
}
          ...
          {companies && companies.items.length > 0 ? (
            <>
              ...
              {companies.total > companies.itemsPerPage ? (
                <Pagination baseUrl="/companies" pageInfo={companies.pageInfo} />
              ) : null}
            </>
          ) : (
