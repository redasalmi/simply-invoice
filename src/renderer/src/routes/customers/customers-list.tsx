...
export async function customersListLoader({ request, context }: LoaderFunctionArgs) {
  ...
  return {
    customers: await window.api.db.customers.get(cursor, paginationType, itemsPerPage),
  }
}
          ...
          {customers && customers.items.length > 0 ? (
            <>
              ...
              {customers.total > customers.itemsPerPage ? (
                <Pagination baseUrl="/customers" pageInfo={customers.pageInfo} />
              ) : null}
            </>
          ) : (
