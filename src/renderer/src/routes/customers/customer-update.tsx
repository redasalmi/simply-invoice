...
import type { UpdateAddressInput, UpdateCustomerWithAddressInput } from '~/types'
...
export async function customerUpdateAction({ request }: ActionFunctionArgs) {
  ...
  const result = await window.api.db.customers.updateWithAddress(customer, address)
      ...
      <section>
        <div>
          <p className="m-12">
            ...
            <Link to="/customers">here</Link> to navigate back to your customers list.
          </p>
        </div>
      </section>
    )
    ...
    <section>
      <Form className="flex flex-col gap-4" method="post" ref={formRef}>
        ...
        <FormField.Root errors={actionData?.errors?.customer?.nested?.name}>
          ...
          <FormField.Input defaultValue={customer.name} name="customer.name" type="text" />
          <FormField.ErrorMessage />
        </FormField.Root>

        <FormField.Root errors={actionData?.errors?.customer?.nested?.email}>
          <FormField.Label>Email</FormField.Label>
          <FormField.Input defaultValue={customer.email} name="customer.email" type="text" />
          <FormField.ErrorMessage />
        </FormField.Root>

        <FormField.Root errors={actionData?.errors?.customer?.nested?.phone}>
          <FormField.Label>Phone</FormField.Label>
          <FormField.Input defaultValue={customer.phone ?? ''} name="customer.phone" type="text" />
          <FormField.ErrorMessage />
        </FormField.Root>

        <FormField.Root errors={actionData?.errors?.customer?.nested?.taxId}>
          <FormField.Label>Tax ID</FormField.Label>
          <FormField.Input defaultValue={customer.taxId ?? ''} name="customer.taxId" type="text" />
          <FormField.ErrorMessage />
        </FormField.Root>

        <Select
          defaultSelectedItem={statusOptions.find((option) => option.value === customer.status)}
        ...
        <div>
          <div>
            ...
            <p className="mb-2 block text-sm">Add additional information about the customer</p>
          </div>
