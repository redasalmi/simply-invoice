...
import type { CreateAddressInput, CreateCustomerWithAddressInput } from '~/types'
...
export async function customerCreateAction({ request }: ActionFunctionArgs) {
  ...
  const result = await window.api.db.customers.createWithAddress(customer, address)
    ...
    <section>
      <Form className="flex flex-col gap-4" method="post" ref={formRef}>
        ...
        <div className="flex flex-col gap-4">
          ...
          <AddressFormFields className="flex flex-col gap-4" errors={actionData?.errors?.address} />
