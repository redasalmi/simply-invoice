...
async function getPreviousCustomers(cursor: string | null, itemsPerPage: number) {
...
export async function deleteCustomer(customerId: string) {
  return db.delete(customersTable).where(eq(customersTable.customerId, customerId))
}
...
export async function registerCustomersIcpHandles() {
  ...
  ipcMain.handle(
    ...
    (_, customer: CreateCustomerWithAddressInput, address: CreateAddressInput) => {
      let customerErrors: CustomerCreateWithAddressFlatErrors | null = null
      let addressErrors: AddressCreateFlatErrors | null = null

      const parsedCustomer = v.safeParse(customerCreateWithAddressSchema, customer)
  ...
  ipcMain.handle(
    ...
    (_, customer: UpdateCustomerWithAddressInput, address: UpdateAddressInput) => {
      let customerErrors: CustomerUpdateWithAddressFlatErrors | null = null
      let addressErrors: AddressUpdateFlatErrors | null = null

      const parsedCustomer = v.safeParse(customerUpdateWithAddressSchema, customer)
      if (!parsedCustomer.success) {
        customerErrors = v.flatten(parsedCustomer.issues)
      }
